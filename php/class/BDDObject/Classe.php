...
final class Classe
{
	...
	public function __construct($params=array())
	{
		...
		if(isset($params['owner'])) $this->_owner = $params['owner'];
	}
	...
	public static function getList($params = array())
	{
		...
		try {
			if ($hasUser !== null) {
				if ($onlyOpen) {
					$bdd_result = DB::query("SELECT c.id id, c.nom nom, c.description, c.idOwner idOwner, c.pwd pwd, c.date date, c.ouverte ouverte, u.nom nomOwner, u.prenom prenomOwner, u.pseudo pseudoOwner, u.rank rankOwner, u.email emailOwner FROM (( ".PREFIX_BDD."classes c INNER JOIN ".PREFIX_BDD."users u ON c.idOwner = u.id ) INNER JOIN users m ON m.idClasse = c.id) WHERE ouverte=1 AND m.id=%s", $hasUser);
				} else {
					$bdd_result = DB::query("SELECT c.id id, c.nom nom, c.description, c.idOwner idOwner, c.pwd pwd, c.date date, c.ouverte ouverte, u.nom nomOwner, u.prenom prenomOwner, u.pseudo pseudoOwner, u.rank rankOwner, u.email emailOwner FROM (( ".PREFIX_BDD."classes c INNER JOIN ".PREFIX_BDD."users u ON c.idOwner = u.id ) INNER JOIN users m ON m.idClasse = c.id) WHERE m.id=%s", $hasUser);
				}
			} elseif ($ownerIs !== null) {
				if ($onlyOpen){
					$bdd_result = DB::query("SELECT c.id id, c.nom nom, c.description, c.idOwner idOwner, c.pwd pwd, c.date date, c.ouverte ouverte, u.nom nomOwner, u.prenom prenomOwner, u.pseudo pseudoOwner, u.rank rankOwner, u.email emailOwner FROM (".PREFIX_BDD."classes c INNER JOIN ".PREFIX_BDD."users u ON c.idOwner = u.id) WHERE ouverte=1 AND c.idOwner=%s", $ownerIs);
				} else {
					$bdd_result = DB::query("SELECT c.id id, c.nom nom, c.description, c.idOwner idOwner, c.pwd pwd, c.date date, c.ouverte ouverte, u.nom nomOwner, u.prenom prenomOwner, u.pseudo pseudoOwner, u.rank rankOwner, u.email emailOwner FROM (".PREFIX_BDD."classes c INNER JOIN ".PREFIX_BDD."users u ON c.idOwner = u.id) WHERE c.idOwner = %s", $ownerIs);
				}
			} else {
				if ($onlyOpen){
					$bdd_result = DB::query("SELECT c.id id, c.nom nom, c.description, c.idOwner idOwner, c.pwd pwd, c.date date, c.ouverte ouverte, u.nom nomOwner, u.prenom prenomOwner, u.pseudo pseudoOwner, u.rank rankOwner, u.email emailOwner FROM ".PREFIX_BDD."classes c INNER JOIN ".PREFIX_BDD."users u ON c.idOwner = u.id WHERE ouverte=1");
				} elseif ($forJoin) {
					...
					$bdd_result = DB::query("SELECT c.id id, c.nom nom, c.description, c.idOwner idOwner, c.pwd pwd, c.date date, c.ouverte ouverte, u.nom nomOwner, u.prenom prenomOwner, u.pseudo pseudoOwner, u.rank rankOwner, u.email emailOwner FROM (".PREFIX_BDD."classes c INNER JOIN ".PREFIX_BDD."users u ON c.idOwner = u.id)");
				}
			}
	...
	public function insertion()
	{
		...
		try {
			// Ajout de la classe
			DB::insert(PREFIX_BDD.'classes', array(
				'nom' => $this->nom,
	...
	public function update($params=array(),$updateBDD=true)
	{
		...
		if(isset($params['idOwner'])) { $this->_idOwner = (integer)$params['idOwner']; $bddModif=true; }
		if(isset($params['owner'])) { $this->_owner = $params['owner'];	$bddModif=true; }
		...
		require_once BDD_CONFIG;
		try{
			DB::update(PREFIX_BDD.'classes', array('nom'=>$this->nom, 'description'=>$this->description, 'pwd'=>$this->pwd, 'ouverte'=>$this->ouverte, 'idOwner'=>$this->getOwnerId()),"id=%s",$this->id);
		} catch(MeekroDBException $e) {
